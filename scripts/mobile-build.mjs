import {mkdtempSync,cpSync,symlinkSync,writeFileSync,readFileSync,existsSync,mkdirSync,rmSync} from 'node:fs';
...
if(!JSON.parse(readFileSync(join(root,'capacitor.config.json'),'utf8')).server?.url&&existsSync(join(root,'.env.local'))){const env=readFileSync(join(root,'.env.local'),'utf8');const token=env.match(/^APP_TOKEN=(.+)$/m)?.[1];const url=env.match(/^MOBILE_SERVER_URL=(.+)$/m)?.[1];if(token&&url)writeFileSync(join(stage,'public/connection.json'),JSON.stringify({url,token}));}
...
rmSync(join(root,'out'),{recursive:true,force:true});cpSync(join(stage,'out'),join(root,'out'),{recursive:true});
